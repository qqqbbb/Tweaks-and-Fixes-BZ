...
namespace Tweaks_Fixes
{
    ...
    { 
        ...
        class PDA_Open_Patch
        {
            //[HarmonyPrefix]
            //[HarmonyPatch("Open")]
            static bool OpenPrefix(PDA __instance, PDATab tab, Transform target, PDA.OnClose onCloseCallback, bool __result)
            {
                PDATab requestedTab = tab;
                if (__instance.isInUse || __instance.ignorePDAInput || BelowZeroEndGame.isActive)
                {
                    __result = false;
                    return false;
                }
                ...
                if (requestedTab == PDATab.None)
                {
                    ...
                    if (main2.isShowingMessage && !string.IsNullOrEmpty(main2.id))
                    {
                        ...
                        if (!(id == "PDAEncyclopediaTab"))
                        {
                            ...
                            tab = PDATab.Encyclopedia;
                    }
                }
                __instance.ui.OnOpenPDA(tab, requestedTab);
                ...
                if (__instance.targetWasSet)
                    __instance.activeSqrDistance = (target.transform.position - main1.transform.position).sqrMagnitude + 1f;
                __instance.audioSnapshotInstance = FMODUWE.GetEvent(__instance.audioSnapshot);
            ...
            static bool ClosePrefix(PDA __instance)
            {
                ...
                IQuickSlots quickSlotsParent = main.GetQuickSlotsParent();
                if (quickSlotsParent != null)
                    uGUI.main.quickSlots.SetTarget(quickSlotsParent);
                ...
                __instance.sequence.Set(0.3f, false, new SequenceCallback(__instance.Deactivated));
                if (__instance.audioSnapshotInstance.isValid())
                {
                    int num1 = (int)__instance.audioSnapshotInstance.stop(FMOD.Studio.STOP_MODE.ALLOWFADEOUT);
                ...
                if (__instance.onCloseCallback != null)
                {
                    ...
                    __instance.onCloseCallback = (PDA.OnClose)null;
                    onCloseCallback(__instance);
                }
                //PDA.OnClose onClose = PDA.onClose;
                //if (PDA.onClose != null)
                //    PDA.onClose(__instance);
                __instance.currentRecyclotron = (Recyclotron)null;
                __instance.ui.gameObject.SetActive(false);  // 
                return false;
            }

            //[HarmonyPostfix]
            //[HarmonyPatch("Close")]
            static void ClosePostfix(PDA __instance)
            {
                if (Main.config.instantPDA)
                    __instance.gameObject.SetActive(false);
            }
